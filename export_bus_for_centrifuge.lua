...
while true do
  db_size = 1
  local me = me_controller.getItemsInNetwork()
  if(me ~= nil) then
    for k, v in ipairs(me) do
      ...
      until(tmp)
      print(tmp)
      print(db_size, db.get(db_size))
      print("********")
      for key, value in pairs(db.get(db_size)) do
        print(key, value)
      end
      print("        ")
      if(tmp) then
    ...
    for i = 1, db_size, 1 do
      if(db.get(i).label == min_label) then
        ...
        export_bus.setExportConfiguration(3, 1, db.address, i)
      end
    end
    for i = 1, db_size, 1 do
      db.clear(i)
